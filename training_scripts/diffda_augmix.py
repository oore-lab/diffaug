...
parser.add_argument(
    ...
    '-r',
    type=str,
    default='workdirs/DAM_diffaug.ckpt',
...
class ConcatDataset(torch.utils.data.Dataset):
   def __init__(self, datasets, train_transform, da_transform):
      ...
      self.all_idxs = list(range(lens[0]*len(lens)))
      self.train_transform = train_transform
      self.da_transform = da_transform
   
   def __getitem__(self, i):
      dset_idx = i//self.len 
      sample_idx = i%self.len 
      tr_sample, y = self.datasets[dset_idx][sample_idx]
      da_sample, _ = self.datasets[0][sample_idx]
      return self.train_transform(tr_sample), self.da_transform(da_sample), y
...
def main():
    ...
    train_dataset = AugMixDataset(concat_dataset, preprocess)
    ...
    net.load_state_dict(
            torch.load("workdirs/deepaugment_and_augmix.pth.tar",map_location=f"cpu")['state_dict'])
    ...
    optimizer.load_state_dict(
            torch.load("workdirs/deepaugment_and_augmix.pth.tar",map_location='cpu')['optimizer'])
